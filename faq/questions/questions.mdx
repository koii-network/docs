...
The issue arises when an excessive amount of KOII tokens are staked on a single task, leading to potential penalties (**slash**). Presently, most tasks do not correlate the reward amount with the stake size, meaning that increasing your stake does not necessarily result in higher KOII earnings for these tasks. However, it's important to note that future tasks may indeed reward higher stakes with greater returns.
