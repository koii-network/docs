...
Somehow your task ended in the available tasks, but you still have some KOII staked. Your KOII are safe and staked in the task. It's just a UI issue that when you try to click the play button it is trying to stake KOII again and since you don't have that much KOII so you are not able to stake it again.
...
The issue arises when an excessive amount of KOII tokens are staked on a single task, leading to potential penalties (**got slashed**). Presently, most tasks do not correlate the reward amount with the stake size, meaning that increasing your stake does not necessarily result in higher KOII earnings for these tasks. However, it's important to note that future tasks may indeed reward higher stakes with greater returns.
