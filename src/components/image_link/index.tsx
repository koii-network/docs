...
export function CardSpecial({
	...
	return (
		<a
			...
			style={{ width: `calc((100% / ${cardPerRow}) - 20px)` }}
			target={target}
			rel="noopener noreferrer"
		>
			{SvgImage && <SvgImage className={styles.cardSvg} alt={title} />}
			<div className={styles.textContainer}>
				<div dangerouslySetInnerHTML={{ __html: title || "" }} />
				<div dangerouslySetInnerHTML={{ __html: description || "" }} />
				<a href={`${link.includes("https") ? "" : baseUrl}${link}`}>
					{linkText}
				</a>
			</div>
		</a>
...
export function Card({
	...
	return (
		<a
			...
			target={target}
		>
			{image && <img src={image} className={styles.cardSvg} alt={title} />}
			<div className={styles.textContainer}>
				<div dangerouslySetInnerHTML={{ __html: title }} />
				<div dangerouslySetInnerHTML={{ __html: description }} />
				<a href={`${link.includes("https") ? "" : baseUrl}${link}`}>
