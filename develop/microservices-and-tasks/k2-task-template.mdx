...
For more information on how the Task Flow works, check out the [runtime environment](/develop/microservices-and-tasks/gradual-consensus) docs.
...
- `NamespaceWrappers.js` — contains the interfaces to make API calls to the core of the task-node. It contains all the necessary functions required to submit and audit the work, as well as the distribution lists. Check [here](/develop/microservices-and-tasks/the-namespace-object) to learn more Namespace functions.
...
## Timers
When a task is executed, it goes through different [phases](/develop/microservices-and-tasks/gradual-consensus#how-does-it-work) that occur consecutively, and during each phase, a function is required to be run. Timers are IPC calls on the task-node that call events on the task during a particular time/slot during a task execution period.
...
Before you begin this process, be sure to check your code and write unit tests wherever possible to verify individual core logic functions. Testing using the [docker container](/develop/microservices-and-tasks/testing-locally-with-docker) should be mostly used for consensus flows, as it will take longer to rebuild and re-deploy the docker container.
...
Open the `.env-local` file and make any modifications you need. You can include environment variables that your task expects to be present here, in case you're using [custom secrets](/develop/microservices-and-tasks/keys-and-secrets).
