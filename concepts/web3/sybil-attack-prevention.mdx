...
In order to protect the network againts bad actors, such as attackers that can utilize a large number of "bot" wallets which can create signatures continuously in order to earn a vast majority of newly created tokens with each epoch, we implemented a number of safeguards.
...
As a result, the [K2](https://blog.koii.network/Koii-Announces-K2/) release contains a number of safeguards against "bot" wallets and sybil attacks by giving wallets different weights when viewing particular content.
