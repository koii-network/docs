...
Any node that performs a task for the first time

## Tasks

A piece of executable code that is run on the network, which costs KOII to run a task and the node that runs the task receives KOII for doing so.

## Settlement Layer

It is the “anchor” that provides security for an entire ecosystem
...
This is a downloadable computer program that allows programmers to use some of their computer processing power to accomplish tasks
...
Proof of Real-Traffic are Koii’s attention tracking system. This is the way creators get rewarded for their work and published content.
...
## KOII Task
- <h3 style={{marginTop: '0px'}}> Task Templates </h3>

The task templates are the use cases that help developers write their task in different situations.

- ### Create a Task

Creating a task usually happens when developers use “Create-task-CLI” to package their task code to task node and upload the task configuration information to IPFS or Arweave.

- ### Deploy a Task

We missed this part in docs Deploy a task means after you create a task and test it successfully, developer ask Koii task manager to whitelist their task then user can run the task on the desktop node.
...
A general agreement; Consensus is achieved when all participants of the network agree on the order and content in a decentralized manor
...
A distributed and encoded database that serves as a ledger where transactions records are stored.
...
## ERC-20
A token standard that allows the Application Programming Interface to be implemented for cryptocurrencies. This makes it so every token possesses the same form and value, and grants the exchange, sharing, and transferring of new tokens into a crypto wallet.
...
## Alt-coins
All other cryptocurrency coins other than Bitcoin, an “alternative coin.”
...
An abstract set of instructions designed to efficiently execute
